...
class Minesweeper():
    ...
    def interactive(self, move, cords):
        ...
        if var == 1:
            ...
            # Обработка поражения, если координаты пользователя равны
            if (cords[0], cords[1]) in self.mas_of_mins:
    ...
    # Обработка победы, если координаты флажков соотвествуют кооржинатам мин
    def is_victory(self):
        flag_coords = set()
    ...
    def random_place_of_mins(self, row, col):
        ...
        mas_of_mins = []
        # Цикл работает пока во множестве меньше элементов чем количество мин
        while len(mas_of_mins) < self.count_mins:
            row_min, col_min = random.randint(
                0, self.height - 1), random.randint(0, self.width - 1)
            # Проверка не координаты мины и=с координатами первой открытой клетки
            if (row_min, col_min) != (row, col):
    ...
    def check_act(self, row, col):
        ...
        count_mins = self.check_min_around(row, col)
            ...
            for i in range(row - 1, row + 2):
                for j in range(col - 1, col + 2):
                    if 0 <= i < self.height and 0 <= j < self.width:  # Если координаты не находятся за пределами поля
                        # И значение ячейки равно этому символу (для окончания рекурсии
                        if self.matrix[i][j] == '■':
                            # и правильного подсчета закрытых клеток)
                            # То мы рекурсивно вызываем функцию с этими координатами
                            self.check_act(i, j)
                            self.count_close += 1
