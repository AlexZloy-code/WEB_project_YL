...
from bulls_and_cows import game_handler
...
def bulls_and_cows():
    ...
    if request.method == 'GET':
        ...
        if len(set(num)) != 4:
            ...
            res = game_handler(
                session["b_a_c"]['num'], num, digits, difficulty, session["b_a_c"]['attempts'])
            if res == 'Победа':
                return res
            if len(res) == 1:
                session["b_a_c"]['attempts'].append(res)
            elif len(res) == 2:
                session["b_a_c"]['num'] = res[0]
                session["b_a_c"]['attempts'].append(res[1])
        return render_template('web.html', error='', history=session["b_a_c"], digits=digits)
